...
class _CombatMixin(_ProtocolBase):
    ...
    async def handle_initiative_roll(self, msg: Message, client_id: str) -> Message:
        ...
        persist_error = self._persist_direct_combat_mutation(
            ...
            client_id,
            session_code=session_code,
            command_type='initiative_roll',
            actor_id=combatant_id,
            command_payload=d,
            result_payload={'combatant_id': combatant_id, 'value': value, 'order': order},
    ...
    async def handle_initiative_set(self, msg: Message, client_id: str) -> Message:
        ...
        persist_error = self._persist_direct_combat_mutation(
            ...
            client_id,
            session_code=session_code,
            command_type='initiative_set',
            actor_id=combatant_id,
            command_payload=d,
            result_payload={'combatant_id': combatant_id, 'value': float(value), 'order': order},
            state_before=state_before,
            state_after=state,
        )
        if persist_error:
    ...
    async def handle_initiative_remove(self, msg: Message, client_id: str) -> Message:
        ...
        persist_error = self._persist_direct_combat_mutation(
            ...
            client_id,
            session_code=session_code,
            command_type='initiative_remove',
            actor_id=combatant_id,
            command_payload=d,
            result_payload={'removed': combatant_id, 'order': order},
            state_before=state_before,
            state_after=state,
        )
        if persist_error:
    ...
    async def handle_turn_skip(self, msg: Message, client_id: str) -> Message:
        ...
        state_before_obj = CombatEngine.get_state(session_code)
        state_before = state_before_obj.to_dict() if state_before_obj else None
        skipped = state_before_obj.get_current_combatant() if state_before_obj else None
        ...
        persist_error = self._persist_direct_combat_mutation(
            ...
            client_id,
            session_code=session_code,
            command_type='turn_skip',
            actor_id=skipped.combatant_id if skipped else None,
            command_payload=d,
            result_payload=result,
            state_before=state_before,
            state_after=state,
