...
export function calculateRacialASI(race: string, subrace?: string, racesData?: Record<string, RaceData>): Partial<Record<AbilityScore, number>> {
...
export function getRacialTraits(race: string, subrace?: string, racesData?: Record<string, RaceData>): RaceTrait[] {
  const RACES_SOURCE = racesData || RACES;
  const raceData = RACES_SOURCE[race];
  if (!raceData) return [];
