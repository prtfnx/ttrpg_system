...
const makeResult = (total: number, rolls: number[], modifier = 0, formula = '1d20') => ({
  total, rolls, modifier, formula, timestamp: new Date(),
});
...
describe('DiceRoller — rolling', () => {
  ...
  it('cancels a pending combat roll when unmounted', () => {
    const onRoll = vi.fn();
    const { unmount } = render(<DiceRoller onRoll={onRoll} />);
    fireEvent.click(screen.getByTitle('Roll 1d20'));
    unmount();
    vi.runAllTimers();
