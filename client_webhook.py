...
class WebhookMessageBuffer:
    """Handle webhook message queuing and delivery"""
    def __init__(self):
        self.incoming_queue = asyncio.Queue()
        self.outgoing_queue = asyncio.Queue()
        
    async def add_incoming(self, message: str):
        """Add incoming webhook message to queue"""
        await self.incoming_queue.put(message)
        
    async def get_incoming(self) -> Optional[str]:
        """Get next incoming message, non-blocking"""
        try:
            return await asyncio.wait_for(self.incoming_queue.get(), timeout=0.1)
        except asyncio.TimeoutError:
            return None
            
    async def add_outgoing(self, message: str):
        """Add outgoing message to queue"""
        await self.outgoing_queue.put(message)
        
    async def get_outgoing(self) -> Optional[str]:
        """Get next outgoing message, non-blocking"""
        try:
            return await asyncio.wait_for(self.outgoing_queue.get(), timeout=0.1)
        except asyncio.TimeoutError:
            return None
...
class WebhookClient:
    ...
    def setup_routes(self):
        ...
        async def receive_webhook(request: Request):
            ...
            try:
                ...
                if message:
                    await self.message_buffer.add_incoming(message)
    ...
    async def receive_data(self) -> Optional[str]:
        ...
        return await self.message_buffer.get_incoming()
...
def receive_data(client: WebhookClient) -> Optional[str]:
    ...
    try:
        ...
        return loop.run_until_complete(receive_data_async(client))
    except RuntimeError:
