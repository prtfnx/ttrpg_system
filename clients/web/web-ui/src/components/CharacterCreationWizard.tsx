...
type WizardFormData = CharacterFormData & {
  background: string;
  strength: number;
  dexterity: number;
  constitution: number;
  ...
  function handleNextSkills() {
    // Add any validation for skills if needed
    setStep((s) => s + 1);
  }

  function handleNextIdentity() {
    // Add any validation for identity if needed
    setStep((s) => s + 1);
  }
  function handleBack() {
    setStep((s) => {
  ...
  return (
    <FormProvider {...methods}>
      <div style={{ background: '#fff', borderRadius: 8, padding: 24, minWidth: 340, maxWidth: 420, boxShadow: '0 4px 32px rgba(0,0,0,0.15)' }}>
        ...
        {step === 4 && <SkillsStep onNext={handleNextSkills} onBack={handleBack} />}
        {step === 5 && <IdentityStep onNext={handleNextIdentity} onBack={handleBack} />}
        {step === 6 && (
          <ReviewStep
            data={methods.getValues() as WizardFormData}
            onBack={() => setStep(5)}
            onConfirm={() => handleFinish(methods.getValues() as CharacterFormData)}
          />
        )}
