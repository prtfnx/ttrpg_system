...
export function calculateRacialASI(race: string, subrace?: string): Partial<Record<AbilityScore, number>> {
...
export function getRacialTraits(race: string, subrace?: string): RaceTrait[] {
  const raceData = RACES[race];
  if (!raceData) return [];
