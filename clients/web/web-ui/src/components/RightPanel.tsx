...
export function RightPanel() {
  const [activeTab, setActiveTab] = useState<'characters' | 'entities' | 'chat' | 'lighting' | 'fog' | 'paint' | 'sync'>('sync');
  ...
  return (
    <div className="game-panel right-panel" style={{ height: '100vh', display: 'flex', flexDirection: 'column', background: '#f8fafc', borderLeft: '1px solid #e5e7eb' }}>
      <div className="tabs" style={{ display: 'flex', borderBottom: '1px solid #e5e7eb' }}>
        <button className={activeTab === 'sync' ? 'active' : ''} style={{ flex: 1, padding: '8px', background: activeTab === 'sync' ? '#fff' : 'transparent', border: 'none', borderBottom: activeTab === 'sync' ? '2px solid #6366f1' : 'none', cursor: 'pointer', fontSize: '11px' }} onClick={() => setActiveTab('sync')}>Sync</button>
