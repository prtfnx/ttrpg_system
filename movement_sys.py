...
import ctypes
TIME_TO_DIE = 2000
def check_collision_with_all_sprites(context, sprite):
    for other_sprite in context.sprites_list:
        if other_sprite != sprite and other_sprite != context.selected and other_sprite.collidable:
            
            # Check for collision
            if sdl3.SDL_HasRectIntersectionFloat(ctypes.byref(sprite.frect), ctypes.byref(other_sprite.frect)):
                print(f'other sprite {other_sprite}, and sprite {sprite}, and context {context.selected}')
    ...
    for sprite in cnt.sprites_list:
        ...
        if sprite.moving:
            ...
            if sprite.collidable:
                if check_collision_with_all_sprites(cnt, sprite):
                    print('collision')
        ...
        if sprite.die_timer is not None:
            ...
            if sprite.die_timer <= 0:
                ...
                sprite.die()
                cnt.sprites_list.remove(sprite)
                #sprite.coord_x.value = 0
                #sprite.coord_y.value = 0
        ...
        try:
            frect.w = sprite.original_w * width.value / cnt.base_width * sprite.scale_x
            frect.h = sprite.original_h * height.value / cnt.base_height * sprite.scale_y
