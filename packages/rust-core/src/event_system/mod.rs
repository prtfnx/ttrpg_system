...
impl EventSystem {
    ...
    pub(crate) fn find_sprite_at_position(
        world_pos: Vec2,
    ...
    pub(crate) fn find_sprite<'a>(
        sprite_id: &str,
        layers: &'a HashMap<String, Layer>,
    ) -> Option<(&'a Sprite, &'a str)> {
    ...
    pub(crate) fn find_sprite_mut<'a>(
        sprite_id: &str,
        layers: &'a mut HashMap<String, Layer>,
    ) -> Option<(&'a mut Sprite, &'a str)> {
        for (layer_name, layer) in layers {
    ...
    pub(crate) fn select_sprites_in_area(
        min: Vec2,
        max: Vec2,
        ...
        for (layer_name, layer) in layers {
            ...
            for sprite in &layer.sprites {
                ...
                let intersects = sprite_max.x >= min.x
                    && sprite_min.x <= max.x
                    && sprite_max.y >= min.y
                    && sprite_min.y <= max.y;
