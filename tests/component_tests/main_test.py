...
def SDL_AppInit_func():
    ...
    try:
        imgui_sys = gui_imgui.ImGuiSystem(window, gl_context)
    ...
    test_context.gl_context = gl_context  # Use the existing gl_context
    ...
    return window, gl_context, imgui_sys
...
def impl_pysdl3_init():
    ...
    if sdl3.SDL_Init(sdl3.SDL_INIT_VIDEO | sdl3.SDL_INIT_EVENTS) < 0:
        print(
    ...
    sdl3.SDL_GL_SetAttribute(sdl3.SDL_GL_CONTEXT_FLAGS, SDL_GL_CONTEXT_FORWARD_COMPATIBLE_FLAG)
    sdl3.SDL_GL_SetAttribute(sdl3.SDL_GL_CONTEXT_MAJOR_VERSION, 4)
    sdl3.SDL_GL_SetAttribute(sdl3.SDL_GL_CONTEXT_MINOR_VERSION, 1)
    sdl3.SDL_GL_SetAttribute(sdl3.SDL_GL_CONTEXT_PROFILE_MASK, SDL_GL_CONTEXT_PROFILE_CORE)

    sdl3.SDL_SetHint(sdl3.SDL_HINT_MAC_CTRL_CLICK_EMULATE_RIGHT_CLICK, b"1")
    ...
    if sdl3.SDL_GL_MakeCurrent(window, gl_context) < 0:
        print("Error: SDL_GL_MakeCurrent failed: " + sdl3.SDL_GetError().decode("utf-8"))
        sys.exit(1)

    if not sdl3.SDL_GL_SetSwapInterval(1):
        print(
            "Warning: Unable to set VSync! SDL Error: " + sdl3.SDL_GetError().decode("utf-8")
        )
        sys.exit(1)
...
def main():
    """Main entry point."""
    window, gl_context, imgui_sys = SDL_AppInit_func()
    #imgui_sys= gui_imgui.ImGuiSystem(window,gl_context)
    # Initialize Net thread
    
    if not imgui_sys:
        logger.error("ImGui system failed to initialize, exiting")
        return
    
    running = True
    event = sdl3.SDL_Event()  # Use sdl3.SDL_Event consistently

    while running:
        # Handle events
        while sdl3.SDL_PollEvent(ctypes.byref(event)):  # Use sdl3.SDL_PollEvent
            ...
            try:
                gui_consumed = imgui_sys.process_event(event)
            except Exception as e:
                logger.error(f"ImGui event processing error: {e}")
                gui_consumed = False
            
            # Only process game events if ImGui didn't consume them
            if not gui_consumed:
                # Handle paint events
                if hasattr(paint, 'handle_paint_events'):
                    try:
                        if paint.handle_paint_events(event):
                            continue  # Paint system consumed the event
                    except Exception as e:
                        logger.error(f"Paint event handling error: {e}")
        ...
        # Swap buffers
        sdl3.SDL_GL_SwapWindow(window)  # Use sdl3.SDL_GL_SwapWindow
        
    # Cleanup
    try:
        if hasattr(imgui_sys, 'impl') and imgui_sys.impl:
            imgui_sys.impl.shutdown()
    except Exception as e:
        logger.error(f"ImGui cleanup error: {e}")
